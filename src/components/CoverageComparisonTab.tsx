...
function getProfileValue(
  ...
  if (ct.includes('general liability') || ct === 'gl')
    return profile.glPerOccurrence;
  if (ct.includes('aggregate') && !ct.includes('umbrella'))
    return profile.glAggregate;
  if (ct.includes('umbrella') || ct.includes('excess'))
    return profile.umbrellaLimit;
  if (ct.includes('auto') || ct.includes('automobile'))
    return profile.autoLimit;
...
function compareAmounts(
  required: string,
  yours: string
): Pick<ComparisonRow, 'status' | 'gapAmount'> {
...
function buildBondingRows(
  findings: Finding[],
  profile: CompanyProfile
): ComparisonRow[] {
  const bondingFindings = findings.filter(
    (f) =>
      f.sourcePass === 'risk-overview' &&
      /bond/i.test(`${f.title} ${f.description}`)
  );
...
export function CoverageComparisonTab({
  findings,
}: CoverageComparisonTabProps) {
    ...
    <div className="bg-white rounded-lg border border-slate-200 overflow-hidden">
      ...
      <div className="px-4 py-3 bg-slate-50 border-t border-slate-200">
        <p className="text-xs text-slate-400">
          Comparison based on profile at time of analysis. Re-analyze after
          profile changes to update.
        </p>
      </div>
    </div>
  );
